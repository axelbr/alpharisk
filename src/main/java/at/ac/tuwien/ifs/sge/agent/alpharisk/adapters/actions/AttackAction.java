...
public class AttackAction implements ActionAdapter<RiskAction> {
    ...
    public NDArray toArray(RiskAction action) {
        // Layout: [0, ... source territories ..., N - 1, N ... target territories ..., 2 * N - 1, 1 Unit, 2 Units, 3 Units, End Phase]
        NDArray vector = manager.zeros(new Shape(2L * numberOfTerritories + 4, 1));
    ...
    public RiskAction toAction(NDArray array) {
        if (array.get(new NDIndex(-1)).getInt() == 1) {
    ...
    public Shape actionShape() {
        return new Shape(2L * numberOfTerritories + 4, 1);
    }
}
