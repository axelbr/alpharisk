...
public class AlphaBetaAgent<G extends Game<A, ?>, A> extends AbstractGameAgent<G, A> implements GameAgent<G, A>
{
    ...
    private final Tree<AbGameNode<A>> abTree;
    ...
    public void setUp(final int numberOfPlayers, final int playerId) {
        ...
        this.gameAbTreeComparator = ((Comparator<Tree<AbGameNode<A>>>)((o1, o2) -> this.gameAbNodeEvaluatedComparator.compare(
            o1.getNode(),
            o2.getNode()))).thenComparing((o1, o2) -> this.gameAbNodeMoveComparator.compare(
            o1.getNode(),
            o2.getNode()));
    }
    ...
    public A computeNextAction(final G game, final long computationTime, final TimeUnit timeUnit) {
        ...
        if (this.sortPromisingCandidates(this.abTree, this.gameAbNodeComparator.reversed())) {
            this.log._trace("Yes");
            return (A)((AbGameNode)Collections.max(this.abTree.getChildren(), this.gameAbTreeComparator).getNode()).getGame().getPreviousAction();
        }
        this.log._trace("No");
        this.lastDepth = 1;
        this.excessTime = 2;
        ...
        this.log._debugf(", done with %d alpha cut-off%s, %d beta cut-off%s and %s left.", this.alphaCutOffs,
                         (this.alphaCutOffs != 1) ? "s" : "", this.betaCutOffs, (this.betaCutOffs != 1) ? "s" : "",
                         Util.convertUnitToReadableString(this.ACTUAL_TIMEOUT - (System.nanoTime() - this.START_TIME), TimeUnit.NANOSECONDS, timeUnit));
        this.log.tracef("Tree has %d nodes, maximum depth %d, and an average branching factor of %s", this.abTree.size(),
                        this.depth, Util.convertDoubleToMinimalString(this.averageBranching, 2));
        ...
        if (!this.abTree.getNode().isEvaluated()) {
            this.labelMinMaxTree(this.abTree, 1);
        }
        this.log.debugf("Utility: %.1f, Heuristic: %.1f", this.abTree.getNode().getUtility(),
                        this.abTree.getNode().getHeuristic());
        return (A)((AbGameNode)Collections.max(this.abTree.getChildren(), this.gameAbTreeComparator).getNode()).getGame().getPreviousAction();
    }
    
    private boolean expandNode(final Tree<AbGameNode<A>> tree) {
        if (tree.isLeaf()) {
            final AbGameNode<A> abGameNode = tree.getNode();
            ...
            if (!game.isGameOver()) {
                ...
                for (final A possibleAction : possibleActions) {
                    tree.add(new AbGameNode(game, possibleAction, this.minMaxWeights, abGameNode.getAbsoluteDepth() + 1));
                }
            }
        }
        return !tree.isLeaf();
    ...
    private boolean appearsQuiet(final Tree<AbGameNode<A>> tree) {
        ...
        final List<Tree<AbGameNode<A>>> siblings = tree.getParent().getChildren();
        final double min = Collections.min(siblings, this.gameAbTreeComparator).getNode().getUtility();
        final double max = Collections.max(siblings, this.gameAbTreeComparator).getNode().getUtility();
        return siblings.size() <= 2 || (min < ((AbGameNode)tree.getNode()).getGame().getUtilityValue() && ((AbGameNode)tree.getNode()).getGame().getUtilityValue() < max);
    }
    ...
    private void quiescence(Tree<AbGameNode<A>> tree) {
        ...
        for (node = tree.getNode(); !node.isEvaluated(); node = tree.getNode()) {
            ...
            if (game.isGameOver() || (game.getCurrentPlayer() >= 0 && (isQuiet || this.appearsQuiet(tree)))) {
                ...
                tree.sort(this.gameAbNodeComparator);
                tree = tree.getChild(tree.getChildren().size() / 2);
                isQuiet = true;
            }
        }
        final AbGameNode<A> originalNode = originalTree.getNode();
    ...
    private void evaluateNode(final Tree<AbGameNode<A>> tree) {
        final AbGameNode<A> node = tree.getNode();
        if (tree.isLeaf()) {
            this.quiescence(tree);
        }
        if (!tree.isRoot()) {
            final AbGameNode<A> parent = tree.getParent().getNode();
    ...
    private void labelMinMaxTree(Tree<AbGameNode<A>> tree, int depth) {
        ...
        while (!stack.isEmpty() && (checkDepth++ % 31 != 0 || !this.shouldStopComputation())) {
            tree = stack.peek();
            if (lastParent == tree || tree.getNode().getAbsoluteDepth() >= depth || !this.expandNode(tree)) {
    ...
    private void pushChildrenOntoStack(final Tree<AbGameNode<A>> tree, final Deque<Tree<AbGameNode<A>>> stack) {
        if (((AbGameNode)tree.getNode()).getGame().getCurrentPlayer() == this.playerId) {
            tree.sort(this.gameAbNodeMoveComparator);
        }
        else {
            tree.sort(this.gameAbNodeMoveComparator.reversed());
        }
    ...
    private boolean sortPromisingCandidates(Tree<AbGameNode<A>> tree, final Comparator<AbGameNode<A>> comparator) {
        for (boolean isDetermined = true; !tree.isLeaf() && tree.getNode().isEvaluated() && isDetermined; tree =
            tree.getChild(0)) {
            isDetermined = (isDetermined && tree.getChildren().stream().allMatch(c -> ((AbGameNode)c.getNode()).getGame().getCurrentPlayer() >= 0));
            if (((AbGameNode)tree.getNode()).getGame().getCurrentPlayer() == this.playerId) {
                tree.sort(this.gameAbNodeEvaluatedComparator.reversed().thenComparing(comparator));
            }
            else {
                tree.sort(this.gameAbNodeEvaluatedComparator.reversed().thenComparing(comparator.reversed()));
            }
        }
        return tree.getNode().isEvaluated() && ((AbGameNode)tree.getNode()).getGame().isGameOver();
    }
